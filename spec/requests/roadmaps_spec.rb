...
describe 'roadmaps' do
  ...
  describe '#update' do
    ...
    context 'when change priority' do
      it do
        patch product_roadmap_path(product_id: product.id.to_s, format: :json),
  ...
  it_behaves_like('sign_in_guard') { let(:r) { patch product_roadmap_path(product_id: 1, format: :json) } }
end
