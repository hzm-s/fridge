...
RSpec.describe ProductListQuery do
  ...
  let!(:product_x) { create_product(person: person_a.id, members: [dev_member(person_c.id)]) }
  let!(:product_y) { create_product(person: person_b.id, members: [dev_member(person_c.id)]) }
